...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
    const { name, account , email, password , phoneNumber} = req.body;

    const dat = await User.findOne({acc_no:account});
    if(dat){
        return next(new ErrorHandler("This Wallet Address is already Registered" ,400));
    };
    const user = await User.create({
        name, acc_no:account ,email, password,phoneNumber, avatar: {
            url: "profilepicUrl"
        }
    });
