...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    fetch('http://localhost:4000/api/v1/myCertificates').then(response => response.json()).then(data => console.log(data)).catch(error => console.error(error));
    console.log(error);
    dispatch({ type: LOGIN_FAIL, payload: error.response.data.message });
  }
};
...
export const logout = () => async (dispatch) => {
  try {
    // fetch('http://localhost:4000/api/v1/logout' , { mode : "no-cors" }).then(response => response.json()).then(async (data) => {
    //   console.log(data)
